...
#define define_mcu_version "0032"
...
u16  illuminationTemp[6],illuminationTempCount=0;//光照
...
void SmartConfig(void){
	...
	if(KeySmartConfig[3] == 1)
	{
		...
		if(KeySmartConfig[5]>=2200)//按下时间大于3S
		{
...
void InitModel(void){
	...
	if(!InitModelFlag){
		if(SendModelCnt>500){
			SendModelCnt=0;
			MainLen = sprintf(MainBuffer,"model ymt.flowerpot.v1\r");
			BufferManageWrite(&buff_manage_struct_t,MainBuffer,MainLen);
		}
...
void InitBlue(void){
	if(!InitBleFlag){
		if(InitBleCnt>300){
			InitBleCnt=0;
			MainLen = sprintf(MainBuffer,"%s %s\r","ble_config set 2692",mcu_version);
			BufferManageWrite(&buff_manage_struct_t,MainBuffer,MainLen);
		}
...
//	if(fertility_control_cnt > 1000*60*20)//20分钟
//	{
//	  PBout(1) = 1;
//		if(fertility_control_cnt > ((1000*60*20)+10000))
//		{
//			fertility_control_cnt=0;
//			/*获取肥力**************************/
//			Value = SortMethod2(ADCBuff,GatherRow,0,LostValue);
//			Value = Value/(GatherRow - (LostValue<<1));
//			Value = Value/4096*3.3;//PA0(肥力)
//			
//			if(Value>3.2){//传感器故障
//				Value = SortMethod2(ADCBuff,GatherRow,0,LostValue);
//				Value = Value/(GatherRow - (LostValue<<1));
...
//				}
//				else{
//					fertility = 3556-1111*Value;
//				}
//			}
//			else if(HardwareVersion == 1)
//			{
//				if(Value>2.7 || Value<1)
//				{
//					//传感器故障
//				}
//				else
...
//					{
//						fertility = 46400-32000*Value;
//						fertilityErrCnt = 0;
//					}
//					else if(Value>=1.4 && Value<1.6)
//					{
//						fertility = 9300-5500*Value;
//						fertilityErrCnt = 0;
//					}
//					else if(Value>=1.6 && Value<2.7)
//					{
//						fertility = 1212-445*Value;
//						fertilityErrCnt = 0;
//					}
//				}
//			}
...
//	MainLen = sprintf(MainBuffer,"%s %s %s %s %f\r\n","result","9","1","0",Value);
//	BufferManageWrite(&buff_manage_struct_t,MainBuffer,MainLen);
//	delay_ms(150);
	...
	/*获取湿度*************************/
//	Value = SortMethod2(ADCBuff,GatherRow,1,LostValue);
//	Value = Value/(GatherRow - (LostValue<<1));
...
//			temperature = 0;
	...
	if(sensor_ligntCnt>1000)
	{
		...
		if(HardwareVersion < 3)
		{
			...
			sensor_ligntErrCnt=0;
			IntValue = iic_read_byte(0x03);
	//		if(IntValue == -1){
	//			IntValue = iic_read_byte(0x03);
	//			if(IntValue == -1){
	//				IntValue = iic_read_byte(0x03);
	//				if(IntValue == -1){
	//					sensor_ligntErrCnt=1;
	//				}
	//			}
	//		}
			
			if(IntValue !=-1){
				sensor_lignt_temp[0] = IntValue;
				
				IntValue = iic_read_byte(0x04);
	//			if(IntValue == -1){
	//				IntValue = iic_read_byte(0x04);
	//				if(IntValue == -1){
	//					IntValue = iic_read_byte(0x04);
	...
	//			}
				...
				if(IntValue !=-1){
					sensor_lignt_temp[1] = IntValue;
					sensor_lignt_exponent = (sensor_lignt_temp[0] & 0xF0) >> 4;
					sensor_lignt_mantissa = ((sensor_lignt_temp[0] & 0x0F) << 4) | (sensor_lignt_temp[1] &0x0F); 
					illuminationCopy = sensor_lignt_mantissa * pow(2, sensor_lignt_exponent)  * 0.045;
	...
	//			illuminationCopy=0;
		...
		if(HardwareVersion >= 3){//BH1750
			IntValue = iic_read_byte(0);
			illuminationCopy = IntValue;
		}
		
		if(illuminationCopy != -2 && illuminationCopy != -1 && (illuminationCopy- illumination >=5 || illumination - illuminationCopy>=5))
		{
	...
	if(water_stageCnt>500)
	{
		water_stageCnt=0;
		/*水位************************/
		IntValue = iic2_read_byte();
		if(IntValue == -1){
			IntValue = iic2_read_byte();
			if(IntValue == -1){
				IntValue = iic2_read_byte();
		...
		if(IntValue != -1)
		{
			water_stageCopy = IntValue;
			water_stageCopy = water_stageCopy/2.5;
			if(water_stageCopy>100){
				water_stageCopy = 100;
			}
		
			if(water_stageCopy<30){
				water_stageCopy=0;
			}
			else{
				FloatValue = (float)water_stageCopy;
				FloatValue = FloatValue-30;
				FloatValue = FloatValue/70;
				FloatValue = FloatValue*100;
				water_stageCopy = FloatValue;
			}
			if((water_stageCopy- water_stage >=2 || water_stage - water_stageCopy>=2))
			{
				water_stage = water_stageCopy;
				...
				else if( memcmp(data_parse_struct_t.siid[0],"5",1)==0)
				{
					...
					else if(memcmp(data_parse_struct_t.piid[0],"8",1)==0)//刷新数据指令
					{
						if(memcmp(data_parse_struct_t.value[0],"true",4)==0)
						{
							...
							MainLenCopy = sprintf(MainBuffer+MainLen,"3 4 %d ",fertility);
							MainLen =  MainLen + MainLenCopy;
							
							if(soil_ec_en){
								MainLenCopy = sprintf(MainBuffer+MainLen,"6 7 %d ",fertility);
								MainLen =  MainLen + MainLenCopy;
							}
		...
		else if(data_parse_struct_t.message_type == get_properties)//读取属性
		{
			if(data_parse_len == 1)//单个属性
			{
				...
				else if(memcmp(data_parse_struct_t.siid[0],"6",1)==0)//读取温度  读取湿度  读取光照强度
				{
					...
					{
						MainLen = sprintf(MainBuffer,"%s %s %s %s %d\r","result","6","7","0",fertility);
					}
				}
				//补光灯
				else if(memcmp(data_parse_struct_t.siid[0],"7",1)==0)
			...
			else if(data_parse_len > 1)//多个属性
			{
				...
				if(memcmp(data_parse_struct_t.siid[0],"1",1)==0)
				{
					...
					MainLen = MainLenCopy;
					MainLenCopy = sprintf(MainBuffer+MainLen,"%s %s %s %s ","1","2","0","ymt.flowerpot.v1");
					MainLen = MainLen + MainLenCopy;
				...
				{					
					...
					MainLenCopy = sprintf(MainBuffer+MainLen,"%s %s %s \"%d,%d\"\r","4","4","0",safe_fertility[0],safe_fertility[1]);//肥力
					MainLen = MainLen + MainLenCopy;
				}
				else if(memcmp(data_parse_struct_t.siid[0],"5",1)==0)	
				{					
...
//			MainLen = sprintf(MainBuffer,"%s %s %s %s\r","properties_changed","2","2","false");
	...
	if(testing_time_appear_timer_cnt>EventReportTime || manual_refresh==1 || manual_refresh_one==0){
		...
		if(manual_refresh_one==0){
			...
			if(soil_ec_en){
				MainLenCopy = sprintf(MainBuffer+MainLen,"%s %s %d ","6","7",fertility);
				MainLen =  MainLen + MainLenCopy;
			}
			...
			if(atoi(botany_type)>=19){//植物类型大于19
				MainLenCopy = sprintf(MainBuffer+MainLen,"%s %s %s ","3","7","19");//植物类型
				MainLen = MainLen + MainLenCopy;
			}else{
...
//		}
		//光照
		//illuminationCopy
//		if(illuminationCopy!=illumination)
//		{
...
//	memcpy(dump_ble,define_dump_ble,18);
	...
	for(MainLen=0;MainLen<6;MainLen++)water_stageTemp[MainLen]=0;
	...
	for(MainLen=0;MainLen<6;MainLen++)temperatureTemp[MainLen]=0;
	temperatureTempCount=0;
  ...
  illumination=0,illuminationCopy=0;//光照
	for(MainLen=0;MainLen<6;MainLen++)illuminationTemp[MainLen]=0;
